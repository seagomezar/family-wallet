...
describe('Database Schema & Operations - Extended', () => {
  ...
  describe('seedCategories - extended', () => {
    ...
    it('categories are in correct order (1-3)', async () => {
      await seedCategories();
    ...
    it('monthly targets are all zero (user configures)', async () => {
      await seedCategories();
      const cats = await db.categories.toArray();
    ...
    it('has correct category types distribution', async () => {
      ...
      expect(types.filter((t) => t === 'fixed').length).toBe(1);
      expect(types.filter((t) => t === 'variable').length).toBe(2);
    });
  });
  ...
  describe('Expense CRUD - extended', () => {
    it('queries expenses by categoryId', async () => {
      await db.expenses.bulkAdd([
        {
          id: 'e-1', budgetId: 'b1', categoryId: 'cat-vivienda',
          description: 'Crédito', amount: 5153000, previousAmount: 5153000,
          paymentSource: 'bancolombia', status: 'paid', isRecurring: true,
          createdAt: new Date(), updatedAt: new Date(),
        },
        {
          id: 'e-2', budgetId: 'b1', categoryId: 'cat-mercado',
          description: 'Mercado', amount: 200000, previousAmount: 180000,
          paymentSource: 'bancolombia', status: 'paid', isRecurring: true,
          createdAt: new Date(), updatedAt: new Date(),
        },
        {
          id: 'e-3', budgetId: 'b1', categoryId: 'cat-vivienda',
          description: 'Administración', amount: 500000, previousAmount: 500000,
          paymentSource: 'tc-sebas', status: 'pending', isRecurring: true,
          createdAt: new Date(), updatedAt: new Date(),
        },
      ]);

      const vivienda = await db.expenses.where('categoryId').equals('cat-vivienda').toArray();
      expect(vivienda).toHaveLength(2);
    });

    it('queries by compound index [budgetId+categoryId]', async () => {
      await db.expenses.bulkAdd([
        {
          id: 'e-1', budgetId: 'b-jun', categoryId: 'cat-vivienda',
          description: 'A', amount: 1000, previousAmount: 0,
          paymentSource: 'bancolombia', status: 'paid', isRecurring: false,
          createdAt: new Date(), updatedAt: new Date(),
        },
        {
          id: 'e-2', budgetId: 'b-jun', categoryId: 'cat-mercado',
          description: 'B', amount: 2000, previousAmount: 0,
          paymentSource: 'bancolombia', status: 'paid', isRecurring: false,
          createdAt: new Date(), updatedAt: new Date(),
        },
        {
          id: 'e-3', budgetId: 'b-jul', categoryId: 'cat-vivienda',
    ...
    it('filters by status', async () => {
      await db.expenses.bulkAdd([
        {
          id: 'e-paid', budgetId: 'b1', categoryId: 'cat-vivienda',
          description: 'Paid', amount: 1000, previousAmount: 0,
          paymentSource: 'bancolombia', status: 'paid', isRecurring: false,
          createdAt: new Date(), updatedAt: new Date(),
        },
        {
          id: 'e-pending', budgetId: 'b1', categoryId: 'cat-vivienda',
    ...
    it('updates expense amount', async () => {
      await db.expenses.add({
        id: 'e-upd', budgetId: 'b1', categoryId: 'cat-vivienda',
    ...
    it('deletes expense', async () => {
      await db.expenses.add({
        id: 'e-del', budgetId: 'b1', categoryId: 'cat-vivienda',
  ...
  describe('BankTransactions - extended', () => {
    ...
    it('updates transaction status', async () => {
      ...
      await db.bankTransactions.update('tx-upd', { status: 'accepted', categoryId: 'cat-mercado' });
      const updated = await db.bankTransactions.get('tx-upd');
      expect(updated!.status).toBe('accepted');
      expect(updated!.categoryId).toBe('cat-mercado');
    });
      ...
      await db.bankTransactions.bulkAdd([
        {
          ...
          categoryId: 'cat-mercado', status: 'accepted', importedAt: new Date(),
        },
        {
          id: 'tx-2', importBatch: 'b1', transactionDate: new Date(),
          description: 'B', reference: 'R2', amount: -200, office: '001',
          categoryId: 'cat-transporte', status: 'accepted', importedAt: new Date(),
        },
      ]);

      const gastar = await db.bankTransactions.where('categoryId').equals('cat-mercado').toArray();
  ...
  describe('Export/Import round-trip', () => {
    it('exports and re-imports data identically', async () => {
      ...
      await db.expenses.add({
        id: 'e-export', budgetId: 'b-export', categoryId: 'cat-vivienda',
