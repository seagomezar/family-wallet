    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 md:hidden">
        <div className="mx-auto flex max-w-4xl items-center justify-around py-2">
          <NavLink to="/" icon={<LayoutDashboard className="h-5 w-5" />} label="Inicio" />
          <NavLink to="/gastos" icon={<List className="h-5 w-5" />} label="Gastos" />
          <NavLink to="/categorias" icon={<FolderOpen className="h-5 w-5" />} label="Categorías" />
          <NavLink to="/importar" icon={<Upload className="h-5 w-5" />} label="Importar" />
          <NavLink to="/ajustes" icon={<Settings className="h-5 w-5" />} label="Ajustes" />
        </div>
      </nav>
      ...
      <nav className="fixed top-14 left-0 bottom-0 z-40 hidden w-56 border-r bg-card p-4 md:block">
        <div className="flex flex-col gap-1">
          <DesktopNavLink to="/" icon={<LayoutDashboard className="h-4 w-4" />} label="Dashboard" />
          <DesktopNavLink to="/gastos" icon={<List className="h-4 w-4" />} label="Gastos Mensual" />
          <DesktopNavLink to="/categorias" icon={<FolderOpen className="h-4 w-4" />} label="Categorías" />
          <DesktopNavLink to="/importar" icon={<Upload className="h-4 w-4" />} label="Importar Banco" />
          <DesktopNavLink to="/ajustes" icon={<Settings className="h-4 w-4" />} label="Ajustes" />
        </div>
      </nav>
